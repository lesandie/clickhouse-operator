...
function update_deployment_resource() {
  ...
  yq e -i '.spec.template.spec.containers[0].image |= "{{ if .Values.operator.image.oci_registry }}{{ .Values.operator.image.oci_registry | trimSuffix \"/\" }}/{{ end }}{{ .Values.operator.image.repository }}:{{ include \"altinity-clickhouse-operator.operator.tag\" . }}"' "${file}"
  ...
  yq e -i '.spec.template.spec.containers[1].image |= "{{ if .Values.metrics.image.oci_registry }}{{ .Values.metrics.image.oci_registry | trimSuffix \"/\" }}/{{ end }}{{ .Values.metrics.image.repository }}:{{ include \"altinity-clickhouse-operator.metrics.tag\" . }}"' "${file}"
